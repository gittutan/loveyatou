...
var _move = false,
    ...
    dayStr = ['今天', '明天', '后天'],
    click_msgs_least = ["你有完没完呀？", "你摸了我" + stat_click + "次了，给钱儿", "非礼呀！救命"],
    click_msgs_most = ["我飞！", "我跑呀跑呀跑！~~", "惹不起你，我还躲不起你么？", "干嘛动我呀！", "不要摸我", "干嘛老摸我啊？", "非礼呀！！！！！", "我也要摸你！", "色狼啊啊！"];
$(function() {
    if (isindex) { //如果是主页
        var now = (new Date()).getHours();
        if (now > 0 && now <= 6) {
            showMessage(visitor + ' 夜猫子儿，你会被床封印的！', 6000);
        } else if (now > 6 && now <= 11) {
            showMessage(visitor + ' 早上好，来一发儿？嘻嘻！', 6000);
        } else if (now > 11 && now <= 14) {
            showMessage(visitor + ' 中午了呢，再来一发儿！', 6000);
        } else if (now > 14 && now <= 18) {
            showMessage(visitor + ' 快来快活啊，反正有大把时光！', 6000);
        } else {
...
function initLeader(){
    $("#spig").contextmenu(function (e) {
        ...
        if (e.which == 3) {
            showMessage("<a href=\"http://www.loveyatou.top/\" title=\"首页\">首页</a>", 10000);
        }
        _move = true;
        ...
        if (_move) {
            var x = e.pageX - _x;
            var y = e.pageY - _y;
            var wx = $(window).width() - $(this).width();
            var dy = $(document).height() - $(this).height();
...
function initCartoon() {
    $("#message").mouseover(function () {
        $(this).fadeTo("300", 0.3);
        showMessage(mouseover_msgs[getRandomNumber(mouseover_msgs.length)]);
    }).mouseout(function () {
        $(this).fadeTo("300", 1)
    }).click(function () {
        if (!ismove) {
            ...
            $("#spig").animate(
                {
                    left: document.body.offsetWidth / 2 * (1 + getRandomNumber(move_factor.length)),
                    top: document.body.offsetHeight / 2 * (1 + getRandomNumber(move_factor.length))
                },
                {
                    duration: 500,
                    complete: showMessage(click_msgs[getRandomNumber(click_msgs.length)])
                }
            );
        } else {
            ismove = false;
...
function boredMove() {
    window.setInterval(function () {
        $.getScript('http://php.weather.sina.com.cn/iframe/index/w_cl.php?code=js&day=2&city=&dfc=1&charset=utf-8', function () {
            ...
            $(".spig").animate({
                    left: document.body.offsetWidth / 2 * (1 + move_factor[getRandomNumber(move_factor.length)]),
                    top: document.body.offsetHeight / 2 * (1 + move_factor[getRandomNumber(move_factor.length)])
                },
                {
                    duration: 2000,
                    complete: showMessage(msgs[getRandomNumber(msgs.length)])
                });
        });
    }, 45000);
}
